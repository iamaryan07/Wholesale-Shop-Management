...
#     kpis = {
#         "Customers": len(cached_query("customers")),
#         "Suppliers": len(cached_query("suppliers")),
#         "Products": len(cached_query("products")),
#         "Employees": len(cached_query("employees")),
#         "Orders": len(cached_query("orders")),
#         "Pending Orders": len(execute_query("orders", eq={"status": "Pending"})),
#     }
    
#     # Calculate revenue (sum of payments)
#     payments = cached_query("payments")
#     revenue = sum([p.get("amount", 0) for p in payments]) if payments else 0
...
#     if recent_orders:
#         # Get related data
#         order_df = pd.DataFrame(recent_orders)
        
#         # Get customers and employees for display
...
#         # Add names to orders
#         order_df['customer_name'] = order_df['customer_id'].map(customers)
#         order_df['employee_name'] = order_df['employee_id'].map(employees)
        
#         # Display columns
#         display_cols = ['order_id', 'customer_name', 'employee_name', 'order_date', 'status']
#         st.dataframe(order_df[display_cols], use_container_width=True)
#     else:
...
#     if ftype == "number":
#         return st.number_input(label, value=float(value) if value is not None else 0.0, step=0.01)
#     if ftype == "int":
#         return st.number_input(label, value=int(value) if value is not None else 0, step=1)
...
#     # Query data
#     rows = execute_query(table_name, order=order_by, desc=(order_dir == "DESC"), limit=limit)
